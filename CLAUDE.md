...
- **【基本ルール】ページ種別を「量産して」sitemapに載せる前に、成績が出ている条件まで写せるか確かめる（2026-08-25導入・重大度高）**:
  2026-08-11に「声優ページは全ページ種別で突出して強い（5.9位・CTR9.5%）」というGSC実測を
  根拠に、過去クールの声優ページ4,483件をsitemapへ追加した。実測は正しかったが、
  **その成績が出ていたのは『今期』という条件の下だけ**だった。索引到達後（2026-08-19〜20）に
  声優ページの平均掲載順位は **5.9位 → 47.0位** に崩れ、サイト全体の週次平均も
  **17.88位 → 24.74位** に悪化した。悪化6.86のうち**4.77（70%）がこの面**で、
  表示回数の比率は12.2%・クリックへの寄与は1件だけ。**表示の1割しか占めない面が悪化の7割を作る。**
  平均掲載順位は「順位の良さ」ではなく**露出した（クエリ×ページ）の加重平均**なので、
  低い順位の露出を足すだけで、**既存ページが1つも下がらなくても数字は悪化する**。
  同時期にGoogleが August 2026 spam update（scaled content abuse を標的・2026-08-18〜21）を
  展開しており、手元のデータでは索引到達との切り分けができない（どちらでも打ち手は同じ）。
  したがって新しいページ種別を機械的に量産するときは、①その成績がどの条件下の実測か、
  ②量産後もその条件が成り立つか、③実在する検索語と噛み合うか（「櫻井孝宏 出演作」は
  検索されるが「櫻井孝宏 2023年夏」は検索されない）を先に確かめる。
  検査は`node scripts/check.ts`の「薄い声優ページを索引に載せない」節。
  対処は**索引から外すだけ**にしてページは残す（内部リンクが通っているため）。
  詳細は`docs/operations.md`の㉜と`docs/seo-2026-08-25/`。
  **なお、この件でクリック率も測り直した。順位のついているページのCTRは7.16%（作品ページは
  18.7位で5.21%＝20位前後の期待値の約5倍）で、スニペット側に直すべき欠陥は無い。**
  GSC上の1.66%は、クリックが起きようのない圏外表示1,435回（クリック0）が分母に入っているだけ。
  **「CTRを上げる施策」の実体は「圏外表示を減らす」ことで、掲載順位の対策と同じ打ち手になる。**
  title・descriptionの小手先の変更を提案しないこと（特に`/person/悠木碧/2026/summer`は
  100表示・9クリック・5.8位＝サイト最大の資産なので触らない）。
