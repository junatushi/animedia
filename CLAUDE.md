...
- `node scripts/seo-audit.ts [--markdown]` … オンサイトSEOの定点観測（2026-08-07導入）。
  title幅・作品ページのQ&A数・検索エンジンに出している在庫・クール要約の網羅率を、
  `content/snapshots/`の実データから毎回おなじ物差しで測る。ネットワーク・トークン不要。
  `.github/workflows/seo-daily.yml`が毎日Issueに起票する。方針と根拠は`docs/seo-playbook.md`。
  `check.ts`が「壊れていたら赤にする不変条件」なのに対し、こちらは「悪化しているかもしれない
  傾向」を出すので**exit codeは常に0**（CIを止めない）
...
- **SEOの方針と根拠は `docs/seo-playbook.md` にまとめてある**（2026-08-07導入。110サイト以上の
  一次情報を調査して作成）。**SEO施策を足す前に必ず読む**こと。特に「やらないと決めたこと」
  （llms.txtへの追加投資・FAQPageリッチリザルト狙いの増量・ビッグワード狙い・被リンク依頼など、
  根拠つきで却下済みのもの）を再提案しないため。日次の点検は`node scripts/seo-audit.ts`。
...
- `lib/workTitle.ts` … 作品ページの`<title>`組み立て（2026-08-05導入）。検索結果で切り捨てられない
  幅（`TITLE_WIDTH_BUDGET`。2026-08-07に32→**29**へ引き下げ＝日本語titleの安全ライン）に
  収まる分だけ配信サービス名を入れる。作品名は主キーワードなので削らず、代わりに接尾辞を
  3段階で落とす（「〜はどこで配信？」→「〜の配信」→作品名のみ）。`.tsx`だと
  `node scripts/check.ts`からimportできない（NodeはJSXを解釈しない）ので素の`.ts`に置いてある
- `lib/workFaq.ts` … 作品ページの「実データ由来のQ&A」と「配信サービスの一覧表」（2026-08-07導入）。
  Annictの実データから作れる問い（配信サービス数・独占かどうか・放送開始日・曜日時刻・声優・制作）
  だけを組み立てる。**データが無い質問は作らない**（「不明です」で水増ししない）。
  表形式はAI検索での抽出率が平文の3倍以上（81%対23%）という実測に基づく施策で、
  **表にリンクは入れない**（配信サービスへのリンクはバッジ側が担う。二重リンクにしない）
- `lib/seasonSummary.ts` … シーズンページ冒頭の要約文（2026-08-07導入）。そのクールの
  配信サービス別カバー本数・独占本数を実データから算出して1〜2文にする。それまで
  シーズンページは h1 のあとに文章が1文も無く、64クールのページが機械から見て
  区別しにくかった。過去クールでは現在形で断定しない（`lib/workAvailability.ts`と同じ規律）
