...
- **【基本ルール】404を行き止まりにしない（2026-08-31導入）**: 404そのものは正しい応答だが、
  そこに着地した人には次の行き先が要る。2026-08-31まではNext.jsの既定画面で
  **サイト内リンクが1本も無かった**。404に着地する経路は実在する（㊱で約2,826ページが
  404だった期間の検索結果、㉟で索引から外した声優ページ、Annictから消えた作品）。
  `app/not-found.tsx`が担当する。**日付からURLを組まないこと**（ビルド時に事前生成され得るので
  クール替わりで古いURLを指す）。検査は`verify-production.sh`のI節
