...
- **自動運用のモデル構成**（2026-09-03改訂）: **深く思考する司令塔は Opus／司令塔から委譲される
  調査・実行は Sonnet**、が原則。
  - **監督役（司令塔）**＝セッション／日次巡回ルーティンのメインモデル。**現在は Opus**
    （Fable はプラン対象外。以前は「現在はFable」と書いていたが実態はOpus）。エージェント定義や
    スクリプトに監督役のモデル名を**ハードコードしない**こと（プラン変更でメインモデルが変われば
    全体がそのまま追従する）。
  - **実行役**＝各サブエージェント（`.claude/agents/*.md` の `model:`）で、**全て `sonnet`**。
    以前は品質重視で sns-marketer だけ opus にしていたが、司令塔（Opus）が方針を決め実行を
    Sonnet に委譲する形に統一した（経緯: 司令塔がほぼ全ての実測・スクリプト実行・定型編集まで
    Opus で回しており、Opus 5 の混雑表示が頻発していた。2026-09-03）。委譲の指針は
    `docs/daily-ops.md` の3節。
