...
- `bash scripts/verify-production.sh` … **本番**SSRの実地検査（2026-08-07導入）。公開URLを実際に
  ...
  **2026-09-07に事前検査を追加**。`/`のHTTPコードを先に見て、**402なら「Vercelの無料枠を
  超過して停止」と名指しして即座に打ち切る**（429とその他のコードも区別する）。
  402のときは他の項目が総崩れするので気づけはするが、自動起票されるIssueに
  「トップページが空」としか書かれず**原因が分からない**。直し方がまったく違う
  （コードの不具合 vs 課金の停止）ため切り分けを先に出す。経緯は`docs/operations.md`の㊶-2
- `node scripts/patrol.js` … **逆張り巡回**（2026-08-31導入）。URLを列挙するのではなく
  **URLの「形」を崩して不変条件を確かめる**。`verify-production.sh`が「実在するURLが正しく
  返るか」を見るのに対し、こちらは「知らない壊れ方」を見つけるための道具。実際にこの方式で、
  既存のどの検査にも掛からない事故が3つ出た（公開APIと埋め込みが**502**／10進以外のIDで
  同じ作品が別URLで200／範囲外の年が索引可能）。実在する名前・IDは索引から取り
  **非ASCIIを優先**する（㊱は日本語名にだけ出たのでASCII名だと素通りする）。
  毎日`.github/workflows/verify-production.yml`が回すので**手で実行する必要は無い**。
  外向き通信のある環境でのみ動く。経緯は`docs/operations.md`の㊲
- `node scripts/check-patrol.js` … 上の巡回自身の回帰テスト（2026-08-31導入）。スタブを立てて
  `patrol.js`を実際に走らせ、**落ちるべきときに落ちる**ことを固定する（巡回は「NGを出さなく
  なる」方向に壊れると毎日緑のまま無力化する）。**相手が居ないときに静かに成功しない**ことも
  検査する。ネットワークには出ない。`patrol.js`を触ったら必ず実行する
- `node scripts/freshness.js` … **収集の欠測検知**（2026-08-31導入）。毎日コミットされる
  データ（GSC・行動ログ・クール別初出日）が**来なくなったこと**を見張る。集計する側
  （`seo-report.js`等）はディレクトリの最新ファイルを読むだけなので、収集が何日止まっていても
  平然と動く＝誰も気づけなかった。実測で**3系列すべてに欠測**があった（`docs/operations.md`の㊳）。
  **取り返せるかで扱いを分ける**（`first-seen`は取り返せないので欠測1日でも失敗、GSC・行動ログは
  警告。ただし最新が古すぎれば「止まっている」として失敗）。取り返せない既知の欠測は
  `ACKNOWLEDGED`に**理由つきで記録**する（毎日赤い検査は数日で読まれなくなるため。記録は
  同じ腕の別の日には効かない）。**収集先そのものも走査する**ので、収集を1本増やして見張りを
  足し忘れると落ちる。ネットワークには出ない。毎日`verify-production.yml`が回すので
  手で実行する必要は無い
- `node scripts/check-freshness.js` … 上の欠測検知自身の回帰テスト（2026-08-31導入）。
  仮の収集ディレクトリを渡して`freshness.js`を実際に動かし、**落ちるべきときに落ちる**ことを
  固定する。この道具が壊れていることは**次に収集が止まった日まで分からず、その日のデータは
  もう取り返せない**。ネットワークには出ない。`freshness.js`を触ったら必ず実行する
...
- `node scripts/check-build-size.js` … **デプロイ成果物の大きさの予算**
  ...
  **【2026-09-21・この検査自身が桁で間違っていた】** 導入時の式は
  `10GB × 0.65 ÷ 10件（必ず残る本番デプロイ数）= 650MB`で、同日の実測303MBを
  「予算の47%」＝余裕あり、と報告していた。**同じ日のダッシュボードの実測は41.93GB
  ＝上限の419%**。理由は①「床」しか数えておらず、**保持期間（Hobby既定30日）に
  入ったデプロイは全部残る**ことを勘定していなかった、②**プレビューデプロイを1件も
  数えていなかった**（作業ブランチ32本が生きており、**生きているブランチのプレビューは
  保持期間で消えない**。41.93GBのうち本番で説明できるのは9.3GBだけ）。
  いまの式は`保持件数 = max(必ず残る6件, 保持日数 × 1日あたりの本番デプロイ数)`、
  `予算 = 10GB × 0.65 ÷ 保持件数`。**1日あたりの本番デプロイ数は手で書かず**
  `vercel.json`の除外を読んでgitの履歴を数えて導出する（履歴が浅くて数えられない
  ときは黙って通さず、既定値を使ったと明示する。CIは`fetch-depth: 400`）。
  **`RETENTION_DAYS`はダッシュボードの設定（Project Settings → Security →
  Deployment Retention Policy）と必ず一致させること。ここだけ直すと検査は緑のまま嘘をつく**
  ので、前提を日付つきで毎回出す。超えたときは**保持期間を何日にすれば収まるかも出す**
  （分母がいちばん強く効くのに「小さくしろ」しか言われないと手が伸びない）。
...
- `node scripts/usage-report.js` … **Vercel利用量の「判定」レポート**（2026-09-15導入）。
  ...
  **【2026-09-19・実測で確定】Hobbyプランでは元データが1件も入らない**
  （下の`fetch-vercel-usage.js`を参照）。いまの利用量の確認は
  `usage-check.yml`が判定日に出すIssue＝**ダッシュボードの目視**が担っている。
  このレポートは有料プランに変えた日から自動で動き出す。
...
- `node scripts/fetch-vercel-usage.js` … **Vercel利用量（請求明細）の取得**（2026-09-15導入）。
  ...
  **【2026-09-19・実測で確定】Hobbyプランではこの経路は使えない。**
  トークンを登録して実行したところ`404 {"code":"not_found","message":"Plan not found."}`が
  返った（認証は通っているので401/403ではない）。**Hobbyは無料枠＝請求サイクルを持たない**ため、
  請求を前提にしたこの窓口に該当が無い。設定ミスではなく、待っても直らず、直す手段も無い。
  代わりになる公開APIも見つからなかった。**この1パターンだけは失敗ではなく「省略」として扱う**
  （判定は本文の`Plan not found`で行う。ステータスだけで省略にすると`teamId`の指定ミスを
  黙って飲み込む）。直しようが無いものを毎朝Issueに積むと数日で読まれなくなり、本物の障害まで
  一緒に見逃す（㉔）。**ただし黙って抜けない**（毎回理由を出す）。
  `freshness.js`の`usage`系列は`blockedReason`付きで残してあり`ℹ`表示になる＝
  **赤くならないが消えもしない**。**1件でも入れば通常の欠測検知に戻る**ので、
  有料プランに変えた日に見張りが自動で復活する。経緯は`docs/operations.md`の[51]
...
- **毎日のSEO改善は `docs/seo-operations.md`**（2026-08-19導入）。`node scripts/seo-report.js`を
  ...
  **数字をここに転記しない**（この節は2026-08-19の断面を書き写したまま放置され、
  2026-09-07には実測と大きく食い違っていた＝上の「数字をドキュメントに転記しないこと」を
  この節自身が破っていた）。**重点の面は毎回`seo-report.js`の②で確かめる。**
  変わっていない判断だけ書くと、**作品ページの「量を増やす」方向には投資しない**
  （1ページあたりクリックが声優ページの半分。量ではなく個別のtitle・内容を直す）。
...
- **アフィリエイト運用**（2026-07-18導入）: 提携・リンク登録・月次の報酬額更新（月1回・5分）は
  ...
  **【2026-09-09に判明・実装は09-19】掲載を恒久停止**（回答は09-09だが`active: false`の
  実装は日次巡回停止＝㊿と重なり10日後の09-19まで持ち越された。経緯は`docs/operations.md`
  ㊷の2026-09-21訂正）。Vercelサポートに**2回**問い合わせ、**2回とも
...
- `vercel.json` の**プレビュー停止**（2026-09-21追加・重大度高） …
  ...
  プレビューは保持期間で消えない**。実測（2026-09-21）で41.93GB＝上限の419%まで
  積み上がり、そのうち本番で説明できるのは9.3GBだけ、残り約32GBが作業ブランチ32本の
  プレビューだった。このサイトは**プレビューURLをどこからも使っていない**
  （cron・計測・巡回はすべて本番ドメイン直指定、CIはGitHub Actions内で自前ビルド）ので
  飛ばして失うものが無い。**「productionでなければスキップ」と書かないこと**
  （環境変数が取れなかった日に本番まで丸ごと止まる。落ちる向きを安全側に倒す）。
  **画面には何も出ない壊れ方**なので`node scripts/check.ts`の
  「プレビューのビルドを飛ばしている」が逆戻りを機械的に禁じる。
  **作業ブランチはマージしたら消すこと**（生きている限りプレビューが残り続ける）。
  経緯は`docs/operations.md`の[54]
- `vercel.json` … **表示に使わないデータのコミットで本番デプロイを起こさないための門番**
  ...
  ときの状態に戻る**。
  **2026-09-01に除外を8つへ拡張**（`scripts/`・`.github/`・`.claude/`・`*.md`を追加）。
  導入時の4つはcronが書くデータを止めるには足りていたが、**人の作業コミットが素通り**していた。
  実測（直近30日）でデプロイを起こした82コミットのうち**32件（39%）が`scripts/check.ts`・
  `CLAUDE.md`・ワークフローしか触っておらず、ビルド成果物は1バイトも変わらないのに毎回
  キャッシュを全消去していた**。除外後は同じ30日で50件。
  **除外パスを手で並べない**こと。`node scripts/check.ts`が`git ls-files`でトップレベルと
  `content/`直下を走査し、1件ずつ「デプロイを起こしてよいか」の表と突き合わせる
  （ディレクトリを足したら表に書くまで落ちる／`app/`を誤って除外しても落ちる）。
  `scripts/`を除外してよい前提＝アプリ側が`scripts/`をimportしていないことも同じ節が検査する。
  **限界**: `ignoreCommand`が見るのは`HEAD^..HEAD`の1コミットだけなので、複数コミットのpushで
  最後だけが除外対象だとその前のコード変更は次のデプロイまで出ない（squash mergeなら起きない）。
  経緯は`docs/operations.md`の㉝
...
- **【基本ルール】ページ種別を「量産して」sitemapに載せる前に、成績が出ている条件まで写せるか確かめる（2026-08-25導入／2026-08-31に数値訂正・重大度高）**:
  2026-08-11に「声優ページは全ページ種別で突出して強い（5.9位・CTR9.5%）」というGSC実測を
  根拠に、過去クールの声優ページ4,483件をsitemapへ追加した。実測は正しかったが、
  **その成績が出ていたのは『今期』という条件の下だけ**だった。索引到達後、声優面の
  平均掲載順位は **5.9位 → 38〜40位** に崩れた。新しいページ種別を機械的に量産するときは、
  ①その成績がどの条件下の実測か、②量産後もその条件が成り立つか、③実在する検索語と
  噛み合うか（「櫻井孝宏 出演作」は検索されるが「櫻井孝宏 2023年夏」は検索されない）を
  先に確かめる。検査は`node scripts/check.ts`の「声優ページの索引方針」節。
  対処は**索引から外すだけ**にしてページは残す（内部リンクが通っているため）。
- **【基本ルール】GSCの「途中までの週」と「完全な週」を並べて比較しない（2026-08-31導入・重大度高）**:
  GSCは3日ラグがあるので、最新の週は必ず途中までしか埋まっていない。2026-08-25の診断で
  これを見落とし、**途中の週（4日ぶん）の作品ページ表示966件**と完全な週を並べて
  「悪化6.86位のうち4.77（70%）は声優ページ由来」と結論した。完全な週では作品ページは
  **2,881表示**で、正しくは悪化5.77位のうち**1.74位＝30%**、残り70%は作品ページの
  表示が21倍になったことだった（作品ページは順位を落としておらずクリックの主収入源なので
  直すべき問題ではない）。**部分データで面の構成比を語ると、結論が逆になる。**
  `weeklyByType`の各行は`days`と`partial`を持つ（`scripts/lib/gsc-page-type.js`）ので、
  **比較の前に必ず`partial`を見る**こと。検査は`node scripts/check.ts`の「途中の週に印を付ける」節。
  同じ診断で「ベースラインは17.88位」とも書いたが、あの週は表示181件しかない偶然の産物で、
  **作品ページ単体は当時から21.70位。このサイトが一桁だった時期は一度も無い。**
- **【基本ルール】索引から外す前に、そのページ群がクリックを取っていないか実測する（2026-08-31導入）**:
  上の誤診に続けて「今期のクール以外は全部noindex」を実装したが、28日のページ別実測では
  **過去年の声優ページが37表示・6クリック・平均20.84位**を取っていた
  （前野智昭/2017/winter 4.0位、斉藤壮馬/2024/summer 8.0位、宮野真守/2026/winter 6.6位、
  谷山紀章/2026/winter 1.0位ほか）。全部外すと28日あたり4〜6クリック＝全体の5〜7%を失う。
  「クールの粒度」も粗すぎた（実際の検索語は「宮野真守 2026 アニメ」＝**年**の粒度）。
...
- **【基本ルール】次クールをsitemapに載せ続ける（2026-08-31導入）**:
  ...
  **検索エンジンに1件も知られていなかった**。放送時期（○年○月）は放送開始の3〜11ヶ月前に
  判明し、検索需要はクール開始の約1ヶ月前から立ち上がる（`docs/next-season-coverage.md`）。
  需要の山は年に4回しか来ないので、1回逃すと次は3ヶ月後。`app/sitemap.ts`の
  `nextYearSeason()`が担当する。**次クールに載せるのは作品ページとシーズンページだけ**
  （声優・サービス別はキャストも配信も埋まっていない段階で薄いページを先回りで送ることになる）。
  検査は`node scripts/check.ts`の「次クールをsitemapに載せる」節。
- **【CTRについての結論】スニペットの小手先の変更を提案しない（2026-08-31確認）**:
  順位のついているページのCTRは**9.33%**（10位以内は14.03%・10〜20位は6.14%）で健全。
  GSC上の1.27%は、クリックが起こりようのない平均23.7位の圏外表示5,443件（全体の86%）が
...
- **【基本ルール】動的セグメントの値域を、実データの範囲に閉じる（2026-08-31導入・重大度高）**:
  クール単位の5面（`/season` `/rankings` `/exclusive` `/service/[key]` `/person/[name]`）は
  年の判定が`/^\d{4}$/`だけだったため、**1000〜9999年の9,000通りが全て有効**だった。
  存在しない年のURLを叩くだけで①Annictへライブ取得が飛び ②その空ページがISRに
  書き込まれ ③中身の無いページが索引可能な形で公開される、が同時に起きる。②は
  2026-08-24に本番を丸一日停止させたISR Writes超過（`docs/operations.md`の㉝）と
  **同じ経路で、しかもURL空間が無制限**だった。いまの規則は
  `lib/resolveSeasonParams.ts`の`isSeasonYearInRange`が1箇所で持つ
  （`MIN_SEASON_YEAR`＝2010〜**今年+1**。+1は次クールが年をまたぐため）。
  **新しい動的セグメントを足すときは、値域を先に決めて範囲外を`notFound()`する**こと。
  検査は`node scripts/check.ts`の「エラーページ・空ページを索引に開放しない」節
  （実際に関数を呼んで9件の境界を確かめる）と`scripts/verify-production.sh`のH節。
  経緯は`docs/operations.md`の㊲
- **【基本ルール】取得に失敗したページ・0件のページを index のまま公開しない（2026-08-31導入・重大度高）**:
  上の5面はどれも`try { getSeasonData() } catch { fetchError = ... }`で失敗を握って
  エラーUIを描く＝**HTTPは200のまま**なので、実測で「Annict API がエラーを返しました
  （500）。」という本文のページが`index, follow`で公開されていた。Annict障害中に
  クローラーが来れば**今期のページまで同じ形で索引され、ISRが最大1週間そのHTMLを配る**。
  判定は`lib/indexPolicy.ts`の`shouldIndexSeasonScopedPage`／`robotsFor`が1箇所で持ち、
  各`generateMetadata`がそれを通す。**404にはしない**（一時的な失敗で404を返すと復旧後に
  再クロールされるまで検索結果から消える）。`follow`は残す（内部リンクを殺さない）。
  データが入れば自動的にindexへ戻るので、手当ては要らない。
- **【基本ルール】404を行き止まりにしない。境界は`page.tsx`と同じ階層に置く（2026-08-31導入）**:
  404そのものは正しい応答だが、そこに着地した人には次の行き先が要る。2026-08-31まではNext.jsの
  既定画面で**サイト内リンクが1本も無かった**。404に着地する経路は実在する（㊱で約2,826ページが
  404だった期間の検索結果、㉟で索引から外した声優ページ、Annictから消えた作品）。
  **`app/not-found.tsx`だけでは足りない**（Next.js 14.2実測）。それが拾うのは「どのルートにも
  一致しなかったURL」だけで、`notFound()`経由の404（＝実際に起きる404のほぼ全部）は既定画面のまま。
  1階層浅い`app/<区画>/not-found.tsx`も効かず、**`page.tsx`と同じ階層**に置いたときだけ効いた。
  中身は`components/NotFoundPanel.tsx`が1つだけ持つ。**日付からURLを組まないこと**
  （事前生成され得るのでクール替わりで古いURLを指す）。検査は`node scripts/check.ts`の
  「notFound() を呼ぶページに404の境界がある」（新しいページ種別を足したら自動で効く）
- **【基本ルール】動的セグメントの値は`Number()`の解釈に任せない（2026-08-31導入・重大度高）**:
  作品IDの検証を3つの窓口が各自で`Number(params.id)` + `Number.isInteger`と書いていた。
  **`Number.isInteger`は「安全な整数か」ではなく「整数値か」しか見ない**ので`1e20`も通り、
  実測で`/api/work/99999999999999999999`と`/embed/anime/...`が**502**を返していた。
  さらに`Number()`は16進・指数・小数・先頭ゼロも受け付けるので、`/anime/0x3374`が
  13172に解決して**同じ作品の別URLが200**になっていた。判定は`lib/workId.ts`の
  `parseWorkId`（`/^[1-9][0-9]{0,8}$/`）が1箇所で持つ。経緯は`docs/operations.md`の㊲
- **【基本ルール】検査の対象を手で数えない。走査して導出する（2026-08-31導入・重大度高）**:
  上の`parseWorkId`を入れたとき、窓口を「3つ」と手で数えて`check.ts`に**名指しで**書いた。
  実際には4つあり、`app/anime/[id]/opengraph-image.tsx`が`Number(params.id)`のまま残った
  （移行の目印にしていた`getWorkData`をOG画像だけは使っていないのでgrepにも掛からなかった）。
  **名指しの検査は書いた3つについては正しく動き続けるので、漏れた1件は永久に見つからない。**
  同じ日に同型の取りこぼしが5つ見つかっている（巡回の対象URL／ISR検査の長い裾6本と画像2本＝
  `/studio`165件と`/director`378件が対象外／索引JSONの一致検査に`studios.json`が無い／
  収集の見張りが3系列とも無い）。
  対象は`scripts/lib/app-routes.js`が`app/`を走査して導出する（`page`/`route`/
  `opengraph-image`/`icon`など**Next.jsがURLに割り当てるファイル**を全部返す）。
  `check.ts`も`patrol.js`も、そこから対象を取る。**新しいページ種別・画像ルートを足したとき
  自動で追随する**。データ側も同じで、`content/archive/`は**全ての.jsonに生成元が登録されて
  いることを先に検査してから**1つずつ導出して突き合わせる（登録しなければその時点で落ちる）。
  新しい検査を書くときは、まず「対象をどう導出するか」を決めること。
  経緯は`docs/operations.md`の㊳
...
- **【計測の前提】速さを測る前に、その画面に実データ・実画像が入っているか確かめる（2026-09-06導入・重大度高）**:
  **この作業環境は本番と同じ画面を描けない。** ①`ANNICT_TOKEN`が無いのでトップは作品0件・
  作品ページは500、②過去クールは`content/snapshots/`から描けるが、**`WORK_IMAGE_IDS`(110件)と
  スナップショットの作品(9,002件)の交差は「0件」**＝カードは全部`thumb-empty`で`<img>`が1つも無い。
  これを確かめずにLCPを測り、「ローカル3回とも候補が`h1.brand`だけ＝LCPは最初の描画」と
  結論した（2026-09-05）。実際は**候補が存在しない画面を3回測っていた**だけで、
  実CSS・実webp・実DOM順で作品ページを再現すると**LCP要素は`IMG.detail-hero-img`に替わり
  FCPと別時刻になる**。**数字は正しく出るので、画面を見ても計測ログを見ても気づけない。**
  `node scripts/measure-pages.js`を使うときは、先に対象URLのHTMLを`curl`して
  **`<img>`と作品リンクの件数を数える**こと（0件なら、その計測で言えるのは
  「空のページの速さ」だけ）。経緯は`docs/operations.md`の㊶
- **【計測の前提】圧縮後のバイト数は「成果物全体を圧縮した差分」で測る（2026-09-05導入）**:
  部品を単独で圧縮した値は、その部品が他の場所と重複していると**桁で外れる**。
  実例: インラインCSSが `<style>` とRSCペイロードに二重に載っているのを見つけ、
  部品単体で「brotli 7,885 B＝作品ページの文書の65%」と見積もったが、実装して測ると
  **625 B（−5.2%）**しか減らなかった（圧縮器が2つ目のコピーを重複排除していた）。
  逆に creditNames が予測どおり効いたのは、画面に一度も出ない＝重複が無かったから。
  経緯は`docs/operations.md`の㊶。
- **【方針】サイト全体の設計見直しは提案しない（2026-09-05／2026-09-07に根拠を差し替え）**:
  ~~「大規模な設計変更を正当化する事故は35件中0件」~~ は**循環論法だったので使わない**
  （このプロジェクトは大規模な設計変更を一度もしていないので、母数0で自明に真になる。
  詳細は`docs/operations.md`の㊶-2）。いま方針を支えているのは次の3つ:
  ①㊶が拘束条件と特定した **Active CPU 106%** を移行が解くという**実測がどこにも無い**
  （この環境からは本番に出られず、移行後の値を測る手段が無い）。
  ②**JSの削減余地にはフレームワークの下限がある** — ビルド実測で App Router 自体が
  gzip 約102.7KB あり、最軽量ページではそれが**全体の99%**。自社コードを全部消しても1%しか減らない。
  ③競合実測で、**ゼロから新規参入して上位に割り込んだ個人サイトの実例が見つからなかった**
  （上位の個人サイトは全て運営歴10年前後＝ドメイン年齢は設計変更では買えない）。
  却下済みの案（items配列のSSR除去・`autoSchedule.json`の除外・Crawl-delay・
  Next.js 15/PPR移行・Astro移行・Speculation Rules・Service Worker・
  サービス別「一覧」ページ新設・画像の外部化・Annict以外のデータ源）は
  `docs/operations.md`の㊶に理由がある。ただし**そのうち3件は却下理由が弱い**と
  2026-09-07に判明している（㊶-2の訂正3）ので、**㊶と㊶-2の両方を読んでから提案すること**。
  **「2秒未満」はSEO要件ではない**（Google公式の閾値はLCP 2.5秒・単一のランキング要因ではない）。
  速度への投資はスマホ主体のUXと無料枠を根拠にする。
- 返答は日本語で。
