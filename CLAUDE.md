...
- **アフィリエイト運用**（2026-07-18導入）: 提携・リンク登録・月次の報酬額更新（月1回・5分）は
  ...
  **【2026-09-09〜】掲載を停止中**。Vercelサポートが「アフィリエイトリンクは収益化が
  主目的でなくても商用利用にあたり、Hobbyプランの規約に抵触する」と回答したため、
  `content/affiliate/programs.ts`の全リンクを`active: false`にした。**データは消していない**
  （移行先が決まるか方針が変わればtrueに戻すだけで復旧する）。**リンクの行を消さないこと。**
  ホスティングの移行検討中。経緯と各社の比較は`docs/operations.md`の㊷
