// ───────────────────────────────────────────────────────────────
// 【重要】Vercelサポートから2026-09-09に下記の回答を受け、登録済みの全リンクを
//   active: false にして停止した。ただし active: false への変更自体が入ったのは
//   PR #160（2026-09-19）で、回答から実装まで10日間そのまま稼働していた
//   （原因は㊿の日次巡回停止と重なったこと。経緯は docs/operations.md の㊷訂正）。
...
//       必要となります」
//   同日、Fair Use Guidelines の "primary purpose"（主目的）という限定を挙げて再照会したが、
//   2通目も **成果報酬型のアフィリエイトリンクの掲載を「認められない要素」に明示列挙**した
//   （「主目的」はガイドラインの想定例にすぎず、規約本体の非商用条項で判断する、との説明）。
//   **2回問い合わせて2回とも不可**なので、**「主目的ではないから大丈夫」を根拠に
//   active を true へ戻さないこと**。
//   一方2通目は「アフィリエイトタグを含まない通常の外部リンクとして運営されている状態で
//   あれば完全に非商用」とも明言したので、**いまの状態のままHobbyを継続してよい**
//   （ホスティングの移行検討は終了した）。
//   このサイトには「恒久的にVercelの無料枠を超えない」という運用上の絶対条件があるため、
//   **リンクの掲出だけを止める**（データは消さない）。
