...
### デプロイ直後の1回だけ失敗しうる（2026-08-11）

このエンドポイントは `getSeasonData()` を**直接**呼ぶため、`/api/season` が持っている
CDNエッジキャッシュ（`s-maxage=600`、実測0.096秒）を経由しない。頼れるのは Next.js の
データキャッシュ（`unstable_cache` 15分）だけだが、**このキャッシュのキーにはビルドIDが
入るのでデプロイのたびに空になる**。空の状態で最初に来た `/anime` はAnnictへの実取得に
なり、2秒の上限を超えてデータ無しで返る。

このとき**「見つかりませんでした」と答えてはいけない**。「その作品は存在しない」という
断定になるため（実際に今期の作品でこれが起きた）。取得できなかったことを述べ、再試行を
促す（`lib/discord.ts` の `buildUnavailableReply`）。2回目以降はキャッシュが温まっている
ので通る。`warm-cache.yml`（毎時）とサイトへの実訪問も同じキャッシュを温めるので、
失敗しうる窓はデプロイ直後のごく短い間だけ。
