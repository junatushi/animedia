...
Threadsは X / Bluesky / Mastodon と挙動が違う点が3つあり、どれも実際に投稿が崩れて
...
### ③ 投稿は「コンテナ作成 → 公開」の2段階で、公開は一発では通らない（2026-08-05追記）

Bluesky/Mastodonは1リクエストで投稿できるが、Threadsは①コンテナ（下書き）を作り
②`threads_publish`で公開する2段階。**①の直後に②をやると`Media Not Found`
（OAuthException code 24 / error_subcode 4279009）で落ちる**。しかもこれは
「コンテナがまだサーバー側に行き渡っていない」という**一時的な**エラーなのに、
`is_transient: false` と返ってくるので、レスポンスの見た目からは恒久エラーと区別がつかない。

- 対策1: `GET /{creation-id}?fields=status` をポーリングし`FINISHED`を待つ
  （`waitUntilPublishable`）。**`status`が返ってこないときは「まだ分からない」であって
  「公開してよい」ではない**ので待ち続ける。ここを素通りさせていたのが2026-08-02までの
  障害の原因のひとつ。
- 対策2: それでも公開が`Media Not Found`を返すことがあるため、`publish`は
  5秒→10秒→20秒→30秒のバックオフで最大5回まで再試行する。再試行するのは
  HTTP 429 / 5xx / subcode 4279009 のときだけで、トークン失効などは即座に失敗させる。
- 回帰テスト: `node scripts/check-threads.js`。経緯は`docs/operations.md`の⑦-8。
...
- **投稿ステップが失敗する（`Threads投稿に失敗しました（401等）`）**: 長期アクセストークンが
  ...
  新しい値で登録し直す。
- **`Media Not Found`（400 / code 24 / error_subcode 4279009）で失敗する**: トークンの問題では
  なく、コンテナの公開待ちの問題（上の⑦-③）。再試行を入れてあるので単発では落ちないはずだが、
  それでも落ちるなら`POLL_TRIES`/`PUBLISH_TRIES`（`scripts/post-threads.js`）を増やす方向で
  調整する。**「一時的なエラーだから」とここの再試行や状態待ちを外さないこと**（外した状態で
  2週間に4回、Threadsだけ0投稿になった）。
- **SNS投稿が失敗した日は**「SNS自動投稿が失敗しています」というIssueが自動で立つ。
  どのSNSが落ちたかの表と実行ログへのリンクが入っているので、そこから追う。直したらIssueを
  閉じる（次に失敗したら新しいIssueが立つ）。
