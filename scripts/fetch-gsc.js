...
//   ・3種類のクエリは1件ずつ try/catch で独立させ、1つの失敗で残りを巻き添えにしない
...
const SPECS = [
  { key: "daily", label: "日別", dimensions: ["date"], rowLimit: 1000 },
  { key: "queries", label: "クエリ別", dimensions: ["query"], rowLimit: 100 },
  { key: "pages", label: "ページ別", dimensions: ["page"], rowLimit: 100 },
];
...
async function main() {
  ...
  // 3種類を1件ずつ独立させる。1つ落ちても残りは取りにいく。
  for (const spec of SPECS) {
    try {
      result[spec.key] = await searchAnalytics(token, spec, range);
      console.log(`${spec.label}: ${result[spec.key].length}行`);
    } catch (e) {
