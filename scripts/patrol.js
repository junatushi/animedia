...
function violations(label, kind, r) {
  ...
  // ①-2 対照（全セグメントが正規の値）は必ず200で返る（2026-09-07追加）。
  //
  // buildCases のコメントに「これが200で返らないなら巡回自体が壊れている」と
  // **書いてあったのに、判定が実装されていなかった**。そのため対照が404でも
  // 違反0件・exit 0 のまま緑になっていた。
  //
  // これは㊱（重大度最高）の再発をこの道具が**構造的に見逃す**という意味になる。
  // ㊱は「事前生成した日本語名のページが本番で全て404」で、sitemapに載せていて
  // 404だったのは約2,826ページ。巡回は実在する名前を非ASCII優先で選んで対照に
  // 使っているので、㊱が再発すればここに必ず出るはずだった。
  //
  // 「NGを出さなくなる方向に壊れると毎日緑のまま無力化する」というのが
  // この道具を作った理由そのもの（`docs/operations.md` の㊲）。
  // get() は redirect:"manual" なので、対照が3xxを返すのも「巡回自体が壊れている」側。
  if (label.startsWith("対照/") && r.status !== 200) {
    v.push(`対照が200を返さない（${r.status}）`);
  }
  if (r.status === 200) {
