...
# ── 事前検査: サイトが「止まって」いないか（2026-09-07追加）──────────
#
# 2026-08-24、Vercel Hobby の利用上限（ISR Writes・Fluid Active CPU・
# Fluid Provisioned Memory）を超過して**全ルートが HTTP 402 を返し、本番が
# 丸一日以上停止した**（docs/operations.md ㉝）。
#
# ところが 2026-09-07 に調べたところ、**このリポジトリのどこにも 402 を明示的に
# 検査している箇所が無かった**（`grep -rn "402" scripts/` はコメント3件だけ）。
# 402 のときこのスクリプトは「h1が0個」「作品リンクが0件」といった検査が
# 総崩れして落ちるので**気づけはする**が、自動起票されるIssueには
# 「トップページが空」としか書かれず、**無料枠の超過だと分からない**。
# 直し方がまったく違う（コードの不具合 vs 課金の停止）ので、切り分けを先に出す。
#
# 402 以外にも、全ページが同じコードで死んでいるなら以降の検査は情報を持たない。
# その場合はここで打ち切って、原因を1行で言う。
PRE_CODE=$($CURL -o /dev/null -w "%{http_code}" "$BASE/")
case "$PRE_CODE" in
  402)
    echo
    echo "NG: 本番が HTTP 402 を返しています＝**Vercelの無料枠を超過して停止**しています。"
    echo "    これはコードの不具合ではありません。対処は docs/operations.md の㉝を読むこと。"
    echo "    ① Vercelダッシュボードで超過した項目（ISR Writes / Active CPU / Provisioned Memory）を見る"
    echo "    ② 書き込みを増やすワークフロー（warm-cache・revalidate）を止める"
    echo "       ※ 停止中は関数が起動しないので書き込みは発生しない。止める目的は"
    echo "          **復旧直後の再生成の山**を防ぐこと（実測: 8/25に約49k・8/26に約73k）"
    exit 1
    ;;
  429)
    echo
    echo "NG: 本番が HTTP 429 を返しています（レート制限）。時間をおいて再実行してください。"
    exit 1
    ;;
  200|30[0-9])
    ok "本番が応答している（HTTP ${PRE_CODE}）"
    ;;
  *)
    echo
    echo "NG: 本番のトップが HTTP ${PRE_CODE} を返しています。以降の検査は情報を持たないので打ち切ります。"
    echo "    000 はこの環境から本番へ到達できない（プロキシ遮断・ネットワーク断）ことを意味します。"
    exit 1
    ;;
esac
...
# 【2026-09-22・向きを反転】過去クールは content/snapshots/ から描く。
# lib/dataFreshness.ts は「静的な確定データに取得日という概念は無い」として
# **日付ごと出さない**（過去クールのページに「今日時点」と書くのは事実として誤り。
# 同じ理由で app/sitemap.ts は lastModified を全部捨てている）。
#
# この規則自体は 2026-09-14 に入れたが、当時は旧形式のスナップショットしか無く
# getWorkData が全作品でライブ取得に落ちていたため日付が出続けており、
# 「取得日があること」を要求していたこの行は通ったままだった。スナップショットを
# 再生成して castCreditsComplete が64クール全てに立ち、過去作品が本当に静的経路へ
# 移った瞬間に**設計どおり日付が消え、検査だけが取り残された**（3日連続で赤）。
#
# 向きを反転させるだけだと「サイト全体から文字列が消えた」場合も通ってしまうので、
# 下のC節に「ライブ取得の作品には出ている」対の検査を必ず置くこと。
wantnot "$PAST_HTML" "配信情報の取得日"     "静的な確定データに取得日を名乗っていない"
...
# 取得日も同じ対にする（2026-09-22追加）。B節は「名乗っていないこと」を見るので、
# lib/dataFreshness.ts が壊れて**どのページでも日付を出さなくなった**場合、
# B節だけでは緑のまま通る。今期の作品はライブ取得＝fetchedAt を持つので、
# こちらには出ていることを要求して「消えただけ」と区別する。
want    "$CUR_HTML" "配信情報の取得日"      "ライブ取得の作品には取得日が出ている"
